...
from backend.utils.common import import_string
...
def get_user_serializer():
    ...
    return import_string(backend_settings.USER_SERIALIZER)
...
def get_user_profile_data(user, request):
    """
    ...
    serializer = import_string(backend_settings.USER_SERIALIZER)
    ...
    if request.data.get('student_id'):
        data['student_id'] = request.data.get('student_id')

    elif request.data.get('tutor_id'):
        data['tutor_id'] = request.data.get('tutor_id')
...
def has_user_activate_token():
    ...
    return backend_settings.USER_ACTIVATE_TOKEN_ENABLED
...
def has_user_verify_code():
    ...
    return backend_settings.USER_VERIFY_CODE_ENABLED
...
def get_user_verified(user):
    ...
    return getattr(user, backend_settings.USER_VERIFY_FIELD)
...
def set_user_verified(user, verified=True):
    ...
    setattr(user, backend_settings.USER_VERIFY_FIELD, verified)
    user.save()
...
def generate_uid_and_token(user, token_generator=None, id=''):
    """
...
def generate_uid_and_token_to_reset(user, token_generator=None):
    """
    Generate UID and token from user information
